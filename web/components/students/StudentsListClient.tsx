                ...
                <tbody>
                  ...
                  <tr className="group/cls cursor-pointer hover:bg-gray-50/60 transition-colors" onClick={() => toggle(cls.id)}>
                    ...
                    <td className="px-3 sm:px-6 py-4 text-center">
                      ...
                      <span className={cn('text-sm font-semibold', totalObs === 0 ? 'text-gray-200' : 'text-gray-700')}>
                        {totalObs}
                      </span>
                    </td>
                    ...
                    <td className="px-3 sm:px-6 py-4 text-right">
                      ...
                      {totalAlerts > 0 ? (
                        <span className="text-sm font-semibold text-red-500">{totalAlerts}</span>
                      ) : (
                      ...
                      {canDelete && (
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover/cls:opacity-100">
                          {(cls.grade === 11 || cls.grade === 12) && (
                            <button
                              onClick={() => handleGraduateClass(cls.id, cls.name)}
                              disabled={loadingId === cls.id}
                              title="Выпустить класс"
                  ...
                  {isOpen && cls.students.length > 0 && (
                    <tr className="bg-gray-50/70 border-t border-gray-100">
                      ...
                      <th className="text-left text-[11px] font-medium text-gray-400 px-3 sm:px-6 py-2">Статус</th>
                      <th className="text-center text-[11px] font-medium text-gray-400 px-3 sm:px-6 py-2">Наблюдения</th>
                      <th className="text-right text-[11px] font-medium text-gray-400 px-3 sm:px-6 py-2">Тревожные</th>
                  ...
                  {isOpen && cls.students.map((student) => {
                    ...
                    return (
                      <tr
                        key={student.id}
                        className="border-t border-gray-50 hover:bg-gray-50/50 group/row cursor-pointer"
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-3">
                            <div className={cn(
                              'w-8 h-8 rounded-full flex-shrink-0 overflow-hidden transition-colors',
                              !student.photo_url && (isAtRisk
                            ...
                            <span className="text-sm font-medium text-gray-900 group-hover/row:text-blue-600 transition-colors truncate">
                              {student.full_name}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-3 sm:px-6 py-3">
                          {isAtRisk ? (
                            <span className="inline-flex items-center gap-1 text-xs font-medium text-red-600 bg-red-50 px-2 py-0.5 rounded-full whitespace-nowrap">
                              ...
                              <span className="hidden sm:inline">В зоне риска</span>
                        ...
                        <td className="px-3 sm:px-6 py-3 text-center">
                          <span className={cn('text-xs font-medium', student.obs_count === 0 ? 'text-gray-300' : 'text-gray-600')}>
                            {student.obs_count}
                          </span>
                        </td>
                        ...
                        <td className="px-3 sm:px-6 py-3 text-right">
                          {student.alert_count > 0 ? (
                            <span className="text-xs font-medium text-red-500">{student.alert_count}</span>
                          ) : (
                            <span className="text-xs text-gray-200">—</span>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                          {canDelete && (
                            <button
                              ...
                              className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover/row:opacity-100"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
                          )}
                  ...
                  {isOpen && cls.students.length === 0 && (
                    <tr>
                      <td colSpan={5} className="text-center text-sm text-gray-400 py-6 border-t border-gray-50">
                        Нет учеников
                      </td>
                    </tr>
                  )}
